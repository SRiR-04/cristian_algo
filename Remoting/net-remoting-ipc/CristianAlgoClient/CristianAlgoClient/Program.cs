...
namespace CristianAlgoClient
{
   ...
   */
    class Client
    {
        ...
        {           
            IpcChannel ipcChannel = new IpcChannel(); ///stworzenia kanału komunikajcyjnego w oparciu o protokuł IPC
            ChannelServices.RegisterChannel(ipcChannel, true); ///rejestracaj utworzonego kanału

            Type requiredType = typeof(CristianInterface);  ///pobranie typu wymanaganego przez wspólny interfejs

            CristianInterface remoteObject = (CristianInterface)Activator.GetObject(requiredType, "ipc://serverIpc/GetTime");
            ///zlecenie wykonanie zadania do serwera poprzez połączenie utworzonym kanałem
